...
HEALTHCHECK --interval=20s --timeout=8s --start-period=60s --retries=3 \
  CMD node -e "require('http').get('http://127.0.0.1:1140/api/health', {timeout: 6000}, (res) => { process.exit(res.statusCode === 200 ? 0 : 1) }).on('error', () => process.exit(1)).on('timeout', () => process.exit(1))"
